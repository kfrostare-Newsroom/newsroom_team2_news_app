describe("regular user can only ", () => {
		...
		cy.window()
			.then(window => {
				window.store.dispatch(
					{ type: "AUTHENTICATE", payload: { currentUser: { email: 'thomas@craft.com', role: '' } } }
				)
			})
		cy.get("#2")
			.last()
	...
	it('Authenticated user that is a SUBSCRIBER can se full length of article', () => {
		cy.window().then(window => window.store.dispatch({ type: "AUTHENTICATE", payload: { currentUser: { email: 'thomas@craft.com', role: 'subscriber' } } }))
		cy.get("#2")
			.last()
			.within(() => {
				cy.get("button")
